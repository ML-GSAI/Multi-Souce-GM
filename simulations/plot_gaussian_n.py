...
file_path = "./results/gaussian_v1/n/results_n.csv"  
...
for i, term in enumerate(empirical_terms):
    ax_left.plot(
        df["n"], df[term], label=labels[i], color=colors[i],
...
ax_left.set_xlabel("(b) Sample Size", fontsize=14)
...
ax_left.set_yticks([0.05, 0.10, 0.15, 0.2, 0.25])
# ax_left.set_xticks(df["n"])
ax_left.set_xticks([100, 1000, 5000])
ax_left.grid(False)
...
ax_right.set_yticks([0.10, 0.30, 0.50, 0.70])
...
ax_left.grid(False)
ax_right.grid(False)
ax_left.set_position([0.1, 0.1, 0.8, 0.8]) 

os.makedirs("./figures", exist_ok=True)
plt.savefig("./figures/gaussian_n.png", bbox_inches='tight', pad_inches=0.03)
plt.savefig("./figures/gaussian_n.svg", bbox_inches='tight', pad_inches=0.03)
plt.show()
