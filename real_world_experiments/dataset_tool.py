...
os.environ['HF_HOME'] = f"{os.environ['HOME']}/datas/cache/hf_cache"
...
from training.dataset import ImageNetSpecficClassDataset
...
def open_imagenet(dir_path: str, split: str, level_name: str, class_labels: str, nums: int):
    inet = ImageNetSpecficClassDataset(root=dir_path, split=split,level_name=level_name, class_labels=class_labels, nums=nums)
    return len(inet), inet
#----------------------------------------------------------------------------
...
def open_mnist(images_gz: str, *, max_images: Optional[int]):
    ...
    images = np.pad(images, [(0,0), (2,2), (2,2)], 'constant', constant_values=0) # reshaped to 32x32
...
def open_3channel_mnist(images_gz: str, *, max_images: Optional[int]):
    labels_gz = images_gz.replace('-images-idx3-ubyte.gz', '-labels-idx1-ubyte.gz')
    assert labels_gz != images_gz
    images = []
    labels = []
...
def open_dataset(source, *, max_images: Optional[int]):
    if "," in source:
        ...
        source = source.split(",")
        for sou in source:
            num_files, input_iter = open_dataset(sou, max_images=max_images)
            nums_files.append(num_files)
            dataset_iters.append(input_iter)
        combined = itertools.chain(*dataset_iters)
        return sum(nums_files), combined
    ...
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == 'cifar-10-python.tar.gz':
            ...
            return open_3channel_mnist(source, max_images=max_images)
...
@click.option('--class_labels', help='selected labels', metavar='0,1',  type=str)
@click.option('--ldm', help='use ldm or not', metavar='true or not',  type=bool)
...
#     print(source)
#     print("transformation: ", transform_image)
#     dataset_attrs = None
#
#     labels = []
#     print(class_labels)
...
#
#         if class_labels:
#             if image['label'] not in class_labels:
#                 continue
#
...
#         # if class_labels:
#         #     if image['label'] in class_labels:
...
def main_inet(
    ...
    # load vae for encode -> latent diffusion
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    if ldm:
        vae = diffusers.AutoencoderKL.from_pretrained("stabilityai/stable-diffusion-2-1", subfolder="vae")
    ...
    num_files, input_iter = open_imagenet(source, "train", class_labels)
    archive_root_dir, save_bytes, close_dest = open_dest(dest)
    if resolution is None: resolution = (None, None)
    transform_image = make_transform(transform, *resolution)
    dataset_attrs = None
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        # Apply crop and resize.
        img = transform_image(numpy.array(image[0]))
        ...
        if ldm:
            ...
            latents = latents.cpu().detach().squeeze().numpy()

            latents_bits = io.BytesIO()
            np.save(latents_bits, latents)
            save_bytes(os.path.join(archive_root_dir, archive_fname.replace(".png",".npy")), latents_bits.getbuffer())
            ...
            img.save(image_bits, format='png', compress_level=0, optimize=False)
            save_bytes(os.path.join(archive_root_dir, archive_fname), image_bits.getbuffer())
        labels.append([archive_fname, image[1]] if image[1] is not None else None)
...
@click.option('--resolution', help='Output resolution (e.g., 512x512)', metavar='WxH',  type=parse_tuple)
@click.option('--level_name', help='levels', metavar='level1',  type=str)
@click.option('--class_labels', help='selected labels', metavar='0,1',  type=str)
@click.option('--nums', help='selected labels', metavar='-1 or positive int',  type=int)
...
def convert(
    ...
    if nums is None:
        nums = -1
    # num_files, input_iter = open_dataset(source, max_images=max_images)
    num_files, input_iter = open_imagenet(source, "train", level_name, class_labels, nums)
    archive_root_dir, save_bytes, close_dest = open_dest(dest)
