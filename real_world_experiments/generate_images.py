...
from training.dataset import trans_classid_to_fake
...
def generate_images(
    ...
    **sampler_kwargs,                           # Additional arguments for the sampler function.
    ...
    level_idx = outdir.find('level')
    level_idx = 'level' + outdir[level_idx + len('level')]
    ...
    if isinstance(net, str):
        ...
        if encoder is None:
            ...
            if encoder is None:
                ...
                encoder = dnnlib.util.construct_class_by_name(class_name='training.encoders.SimpleVAEEncoder')
                ...
                if len(r.seeds) > 0:
                    ...
                    if net.label_dim > 0:
                        ...
                        if class_idx is not None:
                            if r.labels.shape[1] <= 10:
                                fake_class_id = trans_classid_to_fake(level_idx, class_idx)
                                r.labels[:, :] = 0
                                r.labels[:, fake_class_id] = 1
                            else:
                                r.labels[:, :] = 0
